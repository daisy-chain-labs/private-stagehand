...
var chatCompletionSystemMessageParamSchema = z3.strictObject({
  content: z3.string().default(""),
  role: z3.literal("system"),
  name: z3.string().optional()
}).strip();
var chatCompletionContentPartTextSchema = z3.strictObject({
  text: z3.string().default(""),
  type: z3.literal("text")
}).strip();
var imageURLSchema = z3.strictObject({
  url: z3.string(),
  detail: z3.union([z3.literal("auto"), z3.literal("low"), z3.literal("high")]).optional()
}).strip();
var chatCompletionContentPartImageSchema = z3.strictObject({
...
var chatCompletionUserMessageParamSchema = z3.strictObject({
  content: chatCompletionContentSchema,
  role: z3.literal("user"),
  name: z3.string().optional()
}).strip();
var functionCallSchema = z3.strictObject({
  arguments: z3.string(),
  name: z3.string()
}).strip();
var functionSchema = z3.strictObject({
  arguments: z3.string(),
  name: z3.string()
}).strip();
var chatCompletionToolMessageParamSchema = z3.strictObject({
  content: z3.string().default(""),
  role: z3.literal("tool"),
  tool_call_id: z3.string()
}).strip();
var chatCompletionFunctionMessageParamSchema = z3.strictObject({
  content: z3.string().default(""),
  name: z3.string(),
  role: z3.literal("function")
}).strip();
var chatCompletionMessageToolCallSchema = z3.strictObject({
  id: z3.string(),
  function: functionSchema,
  type: z3.literal("function")
}).strip();
var chatCompletionAssistantMessageParamSchema = z3.strictObject({
...
var functionDefinitionSchema = z4.strictObject({
  name: z4.string(),
  description: z4.string().optional(),
  parameters: functionParametersSchema.optional()
}).strip();
var chatCompletionToolSchema = z4.strictObject({
...
var openAIModelParamsSchema = z5.strictObject({
  ...
  response_format: z5.strictObject({ type: z5.literal("json_object") }).nullish(),
  tool_choice: z5.union([
    z5.literal("auto"),
    z5.literal("none"),
    z5.strictObject({
      type: z5.literal("function"),
      function: z5.strictObject({ name: z5.string() }).strip()
    }).strip()
  ]).optional(),
...
var anthropicModelParamsSchema = z5.strictObject({
...
var googleModelParamsSchema = z5.strictObject({
  temperature: z5.number(),
  maxOutputTokens: z5.number().optional(),
  topP: z5.number().optional(),
  topK: z5.number().optional()
}).strip();
var jsCompletionParamsSchema = z5.strictObject({}).strip();
...
function generateBaseTableSchema(objectName, opts) {
  ...
  return z6.strictObject({
    id: z6.string().uuid().describe(`Unique identifier for the ${objectName}`),
    project_id: z6.string().uuid().describe(
      `Unique identifier for the project that the ${objectName} belongs under`
    ),
    name: z6.string().describe(nameDescription),
    description: z6.string().nullish().describe(`Textual description of the ${objectName}`),
    ...
    user_id: z6.string().uuid().nullish().describe(`Identifies the user who created the ${objectName}`),
    metadata: z6.record(customTypes.any).nullish().describe(`User-controlled metadata about the ${objectName}`)
  });
}
var userBaseSchema = generateBaseTableSchema("user");
var userSchema = z6.strictObject({
  id: userBaseSchema.shape.id,
  given_name: z6.string().nullish().describe("Given name of the user"),
  family_name: z6.string().nullish().describe("Family name of the user"),
  email: z6.string().nullish().describe("The user's email"),
  avatar_url: z6.string().nullish().describe("URL of the user's Avatar image"),
  created: userBaseSchema.shape.created
}).openapi("User");
var organizationBaseSchema = generateBaseTableSchema("organization");
var organizationSchema = z6.strictObject({
  id: organizationBaseSchema.shape.id,
  name: organizationBaseSchema.shape.name.nullish(),
  api_url: z6.string().nullish(),
  created: organizationBaseSchema.shape.created
}).openapi("Organization");
var memberSchema = z6.strictObject({
  org_id: organizationSchema.shape.id,
  user_id: userSchema.shape.id
}).openapi("Member");
var meSchema = z6.strictObject({
  id: userSchema.shape.id,
  organizations: z6.strictObject({
    id: memberSchema.shape.org_id,
    name: organizationSchema.shape.name
  }).array()
}).openapi("Me");
var apiKeyBaseSchema = generateBaseTableSchema("api key");
var apiKeySchema = z6.strictObject({
...
var projectSchema = z6.strictObject({
  id: projectBaseSchema.shape.id,
  org_id: z6.string().uuid().describe(
...
var datasetSchema = z6.strictObject({
...
var promptSchema = z6.strictObject({
  ...
  tags: z6.array(z6.string()).nullish().describe("A list of tags for the prompt"),
  metadata: promptBaseSchema.shape.metadata
});
var repoInfoSchema = z6.strictObject({
  commit: z6.string().nullish().describe("SHA of most recent commit"),
  branch: z6.string().nullish().describe("Name of the branch the most recent commit belongs to"),
  tag: z6.string().nullish().describe("Name of the tag on the most recent commit"),
  dirty: z6.boolean().nullish().describe(
    "Whether or not the repo had uncommitted changes when snapshotted"
  ),
  author_name: z6.string().nullish().describe("Name of the author of the most recent commit"),
  author_email: z6.string().nullish().describe("Email of the author of the most recent commit"),
  commit_message: z6.string().nullish().describe("Most recent commit message"),
  commit_time: z6.string().nullish().describe("Time of the most recent commit"),
  git_diff: z6.string().nullish().describe(
...
var experimentSchema = z6.strictObject({
  ...
  commit: z6.string().nullish().describe("Commit, taken directly from `repo_info.commit`"),
  base_exp_id: z6.string().uuid().nullish().describe(
    "Id of default base experiment to compare against when viewing this experiment"
  ),
  deleted_at: experimentBaseSchema.shape.deleted_at,
  dataset_id: z6.string().uuid().nullish().describe(
    "Identifier of the linked dataset, or null if the experiment is not linked to a dataset"
  ),
  dataset_version: z6.string().nullish().describe(
    "Version number of the linked dataset the experiment was run against. This can be used to reproduce the experiment after the dataset has been modified."
  ),
  public: z6.boolean().describe(
    "Whether or not the experiment is public. Public experiments can be viewed by anybody inside or outside the organization"
  ),
  user_id: experimentBaseSchema.shape.user_id,
  metadata: experimentBaseSchema.shape.metadata
}).openapi("Experiment");
var permissionEnum = z6.enum([
  "create",
  "read",
  "update",
  "delete",
...
var roleSchema = z6.strictObject({
  id: roleBaseSchema.shape.id,
  org_id: z6.string().uuid().nullish().describe(
    [
  ...
  member_permissions: z6.array(permissionEnum).nullish().describe("Permissions which belong to this role"),
  member_roles: z6.array(z6.string().uuid()).nullish().describe(
    [
...
var groupSchema = z6.strictObject({
  ...
  member_users: z6.array(z6.string().uuid()).nullish().describe("Ids of users which belong to this group"),
  member_groups: z6.array(z6.string().uuid()).nullish().describe(
    [
...
var aclObjectSchema = z6.strictObject({
  id: aclBaseSchema.shape.id,
  object_type: aclObjectTypeEnum,
  object_id: z6.string().uuid().describe("The id of the object the ACL applies to"),
  restrict_object_type: z6.optional(z6.union([aclObjectTypeEnum, z6.null()])).describe(
    "Optionally restricts the permission grant to just the specified object type"
  ),
  _object_org_id: z6.string().uuid().describe("The organization the ACL's referred object belongs to"),
  created: aclBaseSchema.shape.created
});
var aclUserObjectSchema = z6.strictObject({
  user_id: z6.string().uuid().describe("Id of the user the ACL applies to")
});
var aclGroupObjectSchema = z6.strictObject({
  group_id: z6.string().uuid().describe("Id of the group the ACL applies to")
});
var aclPermissionObjectSchema = z6.strictObject({
  permission: permissionEnum.describe("Permission the ACL grants")
});
var aclRoleObjectSchema = z6.strictObject({
  role_id: z6.string().uuid().describe("Id of the role the ACL grants")
});
var aclSchema = z6.union([
...
var appLimitSchema = z6.number().int().nonnegative().describe("Limit the number of objects to return");
function generateBaseTableOpSchema(objectName) {
  return z6.strictObject({
    org_name: z6.string().nullish().describe(
...
var startingAfterSchema = z6.string().uuid().describe(
  [
    "Pagination cursor id.",
    "For example, if the final item in the last page you fetched had an id of `foo`, pass `starting_after=foo` to fetch the next page. Note: you may only pass one of `starting_after` and `ending_before`"
  ].join("\n\n")
).openapi("StartingAfter");
var endingBeforeSchema = z6.string().uuid().describe(
  [
    "Pagination cursor id.",
...
function makeObjectIdsFilterSchema(objectName) {
  return z6.string().uuid().describe(
...
var createProjectSchema = z6.strictObject({
  name: projectSchema.shape.name,
  org_name: createProjectBaseSchema.shape.org_name
}).openapi("CreateProject");
var patchProjectSchema = z6.strictObject({
  name: projectSchema.shape.name.nullish()
}).openapi("PatchProject");
var createExperimentSchema = z6.strictObject({
  ...
  ensure_new: z6.boolean().nullish().describe(
...
var createDatasetSchema = z6.strictObject({
...
var patchDatasetSchema = z6.strictObject({
...
var patchPromptSchema = z6.strictObject({
...
var createRoleSchema = z6.strictObject({
...
var createGroupSchema = z6.strictObject({
...
function generateBaseEventOpSchema(objectType) {
  ...
  return z7.strictObject({
    ...
    [TRANSACTION_ID_FIELD]: z7.string().describe(
    ...
    context: z7.strictObject({
      caller_functionname: z7.string().nullish().describe(
        `The function in code which created the ${eventDescription} event`
      ),
      caller_filename: z7.string().nullish().describe(
        `Name of the file in code where the ${eventDescription} event was created`
      ),
      caller_lineno: z7.number().int().nullish().describe(
    ...
    span_parents: z7.string().array().nullish().describe(
    ...
    root_span_id: z7.string().describe(
      `The \`span_id\` of the root of the trace this ${eventDescription} event belongs to`
    ),
    span_attributes: z7.strictObject({
      name: z7.string().nullish().describe("Name of the span, for display purposes only"),
      type: z7.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.any).nullish().describe(
      "Human-identifying attributes of the span, such as name, type, etc."
    ),
    [OBJECT_DELETE_FIELD]: z7.boolean().nullish().describe(
...
function generateBaseEventFeedbackSchema(objectType) {
  ...
  return z7.strictObject({
    ...
    scores: z7.record(z7.number().min(0).max(1).nullish()).nullish().describe(
    ...
    tags: z7.array(z7.string()).nullish().describe("A list of tags to log"),
    comment: z7.string().nullish().describe(
...
var fetchLimitSchema = z7.number().int().nonnegative().describe(
  [
...
var maxXactIdSchema = z7.string().describe(fetchPaginationCursorDescription);
var maxRootSpanIdSchema = z7.string().describe(fetchPaginationCursorDescription);
var versionSchema = z7.string().describe(
  [
...
var pathTypeFilterSchema = z7.strictObject({
  type: z7.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: z7.string().array().describe(
...
var sqlTypeFilterSchema = z7.strictObject({
  type: z7.literal("sql_filter").describe("Denotes the type of filter as a sql-type filter"),
  expr: z7.string().describe(
...
var allFetchFiltersSchema = z7.union([pathTypeFilterSchema, sqlTypeFilterSchema]).array().describe(
...
var fetchEventsRequestSchema = z7.strictObject({
...
var experimentEventSchema = z7.strictObject({
  id: experimentEventBaseSchema.shape.id,
  dataset_record_id: z7.string().nullish().describe(
...
var datasetEventSchema = z7.strictObject({
...
var promptSessionEventSchema = z7.strictObject({
...
var projectLogsEventSchema = z7.strictObject({
  ...
  log_id: z7.literal("g").describe("A literal 'g' which identifies the log as a project log"),
...
var mergeEventSchema = z7.strictObject({
  [IS_MERGE_FIELD]: customTypes.literalTrue.describe(isMergeDescription),
  [MERGE_PATHS_FIELD]: z7.string().array().array().nullish().describe(
    [
...
var replacementEventSchema = z7.strictObject({
  [IS_MERGE_FIELD]: customTypes.literalFalse.nullish().describe(isMergeDescription),
  [PARENT_ID_FIELD]: z7.string().nullish().describe(
    [
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = z7.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
  const requestSchema = z7.strictObject({
...
var insertEventsResponseSchema = z7.strictObject({
  row_ids: z7.string().array().describe(
...
var feedbackExperimentItemSchema = z7.strictObject({
...
var feedbackDatasetItemSchema = z7.strictObject({
...
var feedbackProjectLogsItemSchema = z7.strictObject({
...
var feedbackPromptItemSchema = z7.strictObject({
...
var feedbackPromptSessionItemSchema = z7.strictObject({
...
function makeCrossObjectIndividualRequestSchema(objectType) {
  ...
  const insertObject = z7.strictObject({
  ...
  return z7.record(z7.string().uuid(), insertObject).nullish().describe(
...
function makeCrossObjectIndividualResponseSchema(objectType) {
  return z7.record(z7.string().uuid(), insertEventsResponseSchema).nullish().describe(
    `A mapping from ${objectType} id to row ids for inserted \`events\``
  );
}
var crossObjectInsertRequestSchema = z7.strictObject({
...
var crossObjectInsertResponseSchema = z7.strictObject({
...
var summarizeScoresParamSchema = z7.boolean().describe(
  "Whether to summarize the scores and metrics. If false (or omitted), only the metadata will be returned."
);
var comparisonExperimentIdParamSchema = z7.string().uuid().describe(
...
var summarizeDataParamSchema = z7.boolean().describe(
  "Whether to summarize the data. If false (or omitted), only the metadata will be returned."
);
var summarizeExperimentResponseSchema = z7.strictObject({
  project_name: z7.string().describe("Name of the project that the experiment belongs to"),
  experiment_name: z7.string().describe("Name of the experiment"),
  project_url: z7.string().url().describe("URL to the project's page in the Braintrust app"),
  experiment_url: z7.string().url().describe("URL to the experiment's page in the Braintrust app"),
  comparison_experiment_name: z7.string().nullish().describe("The experiment which scores are baselined against"),
  scores: z7.record(
    z7.strictObject({
      name: z7.string().describe("Name of the score"),
      score: z7.number().min(0).max(1).describe("Average score across all examples"),
      diff: z7.number().min(-1).max(1).describe(
        "Difference in score between the current and comparison experiment"
      ),
      improvements: z7.number().int().min(0).describe("Number of improvements in the score"),
      regressions: z7.number().int().min(0).describe("Number of regressions in the score")
    }).describe("Summary of a score's performance").openapi("ScoreSummary")
  ).nullish().describe("Summary of the experiment's scores"),
  metrics: z7.record(
    z7.strictObject({
      name: z7.string().describe("Name of the metric"),
      metric: z7.number().describe("Average metric across all examples"),
      unit: z7.string().describe("Unit label for the metric"),
      diff: z7.number().describe(
        "Difference in metric between the current and comparison experiment"
      ),
      improvements: z7.number().int().min(0).describe("Number of improvements in the metric"),
      regressions: z7.number().int().min(0).describe("Number of regressions in the metric")
...
var summarizeDatasetResponseSchema = z7.strictObject({
  project_name: z7.string().describe("Name of the project that the dataset belongs to"),
  dataset_name: z7.string().describe("Name of the dataset"),
  project_url: z7.string().url().describe("URL to the project's page in the Braintrust app"),
  dataset_url: z7.string().url().describe("URL to the dataset's page in the Braintrust app"),
  data_summary: z7.strictObject({
    total_records: z7.number().int().min(0).describe("Total number of records in the dataset")
