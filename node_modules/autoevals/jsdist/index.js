...
function buildClassificationFunctions(useCoT, choiceStrings) {
...
var DEFAULT_RAGAS_MODEL = "gpt-3.5-turbo-16k";
...
var ContextEntityRecall = async (args) => {
  var _a;
...
var ContextRelevancy = async (args) => {
  const { chatArgs, client, ...inputs } = parseArgs(args);
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: import_mustache2.default.render(SENTENCE_PROMPT, { question: input, context })
      }
    ],
    tools: [
      {
...
var ContextPrecision = async (args) => {
  const { chatArgs, client, ...inputs } = parseArgs(args);
  const { input, expected, context } = checkRequired(
    { input: inputs.input, expected: inputs.expected, context: inputs.context },
    "ContextPrecision"
...
var AnswerRelevancy = async (args) => {
  var _a;
  const { chatArgs, client, ...inputs } = parseArgs(args);
  const { input, context, output } = checkRequired(
    { input: inputs.input, context: inputs.context, output: inputs.output },
...
var AnswerSimilarity = async (args) => {
  const { chatArgs, client, ...inputs } = parseArgs(args);
...
var AnswerCorrectness = async (args) => {
  var _a, _b, _c, _d;
  const { chatArgs, client, ...inputs } = parseArgs(args);
...
function parseArgs(args) {
  ...
  const chatArgs = {
    model: (_a = args.model) != null ? _a : DEFAULT_RAGAS_MODEL,
